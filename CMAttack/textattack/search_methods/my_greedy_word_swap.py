...
class MyGreedyWordSwapWIR(SearchMethod):
    ...
    def _get_index_order(self, initial_text, max_len=-1):
        ...
        # 使用pd.read_csv读取存储在self.path_to_olm_csv路径下的csv文件，并将其存储在df_olm变量中。
        df_olm = pd.read_csv(self.path_to_olm_csv, encoding='gbk')
        # 获取索引映射
        index_of_mapping = MyGreedyWordSwapWIR.index
        MyGreedyWordSwapWIR.index += 1

        # 使用eval语句，将索引映射应用于df_olm的relevances列，以获取对应的单词重要性分数，并将其存储在mapped_scores变量中。
    ...
    def perform_search(self, initial_result):
        ...
        # 获取单词的重要性排序和search_over变量的值。
        index_order, search_over = self._get_index_order(attacked_text)
        # 然后，使用一个循环来迭代每个单词重要性顺序，直到达到索引顺序的末尾或search_over变量的值为True。
        i = 0
