...
def get_datasets_dataset_columns(dataset):
    """Common schemas for dataset found in dataset hub."""
...
class HuggingFaceDataset(Dataset):
    ...
    Args:
        name_or_dataset (:obj:`Union[str, dataset.Dataset]`):
            The dataset name as :obj:`str` or actual :obj:`dataset.Dataset` object.
            If it's your custom :obj:`dataset.Dataset` object, please pass the input and output columns via :obj:`dataset_columns` argument.
        subset (:obj:`str`, `optional`, defaults to :obj:`None`):
            The subset of the main dataset. Dataset will be loaded as :obj:`dataset.load_dataset(name, subset)`.
        ...
        label_names (:obj:`list[str]`, `optional`, defaults to :obj:`None`):
            ...
            If not set, labels will printed as is (e.g. "0", "1", ...). This should be set to :obj:`None` for non-classification dataset.
        output_scale_factor (:obj:`float`, `optional`, defaults to :obj:`None`):
            ...
            Some dataset are regression tasks, in which case this is necessary.
    ...
    def __init__(
        ...
        if isinstance(name_or_dataset, datasets.Dataset):
            ...
            textattack.shared.logger.info(
                f"Loading {_cb('dataset')} dataset {_cb(self._name)}{subset_print_str}, split {_cb(split)}."
            )
    ...
    def filter_by_labels_(self, labels_to_keep):
        """Filter items by their labels for classification dataset. Performs
        in-place filtering.
