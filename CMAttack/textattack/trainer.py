...
class Trainer:
    ...
    Example::
        ...
        >>> train_dataset = textattack.dataset.HuggingFaceDataset("imdb", split="train")
        >>> eval_dataset = textattack.dataset.HuggingFaceDataset("imdb", split="test")
    ...
    def __init__(
        ...
        if train_dataset:
            ...
            ), f"`train_dataset` must be of type `textattack.dataset.Dataset`, but got type `{type(train_dataset)}`."

        if eval_dataset:
            assert isinstance(
                eval_dataset, textattack.datasets.Dataset
            ), f"`eval_dataset` must be of type `textattack.dataset.Dataset`, but got type `{type(eval_dataset)}`."
        ...
        for epoch in range(1, self.training_args.num_epochs + 1):
            ...
            if self.attack and epoch > num_clean_epochs:
                if (
                    ...
                    # adv_dataset is instance of `textattack.dataset.Dataset`
                    model.eval()
    ...
    def _write_readme(self, best_eval_score, best_eval_score_epoch, train_batch_size):
        ...
        if dataset_name:
            dataset_str = (
                "and the `{dataset_name}` dataset loaded using the `dataset` library"
            )
        else:
            dataset_str = ""
