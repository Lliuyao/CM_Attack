...
class Attacker:
    ...
    Example::
        ...
        >>> dataset = textattack.dataset.HuggingFaceDataset("imdb", split="test")
    ...
    def __init__(self, attack, dataset, attack_args=None):
        ...
        ), f"`dataset` must be of type `textattack.dataset.Dataset`, but got type `{type(dataset)}`."
    ...
    def save_successful_attacks_to_csv(self, file_path):
        """保存攻击成功的样本到 CSV 文件。
        Args:
            file_path (str): 要保存到的 CSV 文件路径。
        """
        successful_attacks = [result for result in self.attack_log_manager.results if
                              isinstance(result, SuccessfulAttackResult)]

        if not successful_attacks:
            print("No successful attacks to save.")
            return
        ...
        with open(file_path, mode='w', newline='') as file:
            ...
            # 写入 CSV 文件头部
            writer.writerow(
                ["Original Text", "Perturbed Text", "Ground Truth", "Perturbed Prediction"])

            for attack_result in successful_attacks:
                original_text = attack_result.original_result.attacked_text.text
                perturbed_text = attack_result.perturbed_result.attacked_text.text
                ground_truth = attack_result.original_result.output
                # original_prediction = attack_result.original_result.output
                perturbed_prediction = attack_result.perturbed_result.output

                writer.writerow(
                    [original_text, perturbed_text, ground_truth, perturbed_prediction])

        print(f"Successful attacks saved to {file_path}.")
