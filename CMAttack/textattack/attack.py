...
class Attack:
    ...
    def __init__(
        ...
        self.constraints = [MyMaxModificationRateConstraint(0.2)]
...
class MyMaxModificationRateConstraint(Constraint):
   def __init__(self, max_rate, min_threshold=3):
       assert isinstance(max_rate, float), "`max_rate` must be a float."
       assert max_rate >= 0 and max_rate <= 1, "`max_rate` must be between 0 and 1."
       assert isinstance(min_threshold, int), "`min_threshold` must be an int."

       self.max_rate = max_rate
       self.min_threshold = min_threshold
       self.compare_against_original = True  # 添加 compare_against_original 属性

   def _check_constraint(self, current_text, transformed_text):
       threshold = max(
